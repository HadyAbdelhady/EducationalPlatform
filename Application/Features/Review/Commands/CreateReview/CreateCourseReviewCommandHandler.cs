...
{
    public class CreateCourseReviewCommandHandler(IUnitOfWork unitOfWork) : IRequestHandler<CreateCourseReviewCommand, Result<CourseReviewCreationResponse>>
    {
        private readonly IUnitOfWork _unitOfWork = unitOfWork;

        public async Task<Result<CourseReviewCreationResponse>> Handle(CreateCourseReviewCommand request, CancellationToken cancellationToken)
        {
            ...
            {
                await _unitOfWork.RollbackTransactionAsync(cancellationToken);
                return Result<CourseReviewCreationResponse>.FailureStatusCode(auth.Message, ErrorType.UnAuthorized);
            }
            catch(Exception ex)
            {
                await _unitOfWork.RollbackTransactionAsync(cancellationToken);
                return Result<CourseReviewCreationResponse>.FailureStatusCode($"Error creating course review: {ex.Message}", ErrorType.InternalServerError);
            }
        }
    }
}
