    ...
    public class CreateSheetCommandHandler(IUnitOfWork unitOfWork, ICloudinaryCore cloudinaryService) : IRequestHandler<CreateSheetCommand, Result<SheetCreationResponse>>
    {
        ...
        public async Task<Result<SheetCreationResponse>> Handle(CreateSheetCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                {
                    var sectionExists = await _unitOfWork.Repository<Domain.Entities.Section>().AnyAsync(s => s.Id == request.SectionId, cancellationToken);
                ...
                {
                    var courseExists = await _unitOfWork.Repository<Domain.Entities.Course>().AnyAsync(c => c.Id == request.CourseId, cancellationToken);
                ...
                var newSheet = new Domain.Entities.Sheet()
                {
                   Id = new Guid(),
                   Name = request.Name,
                   Type = request.Type,
                ...
                await _unitOfWork.Repository<Domain.Entities.Sheet>().AddAsync(newSheet, cancellationToken);
