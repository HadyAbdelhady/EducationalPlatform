...
namespace Application.Features.Sheet.Commands.DeleteSheet
{
    public class DeleteSheetCommandHandler(IUnitOfWork unitOfWork, ICloudinaryCore cloudinaryService) : IRequestHandler<DeleteSheetCommand, Result<string>>
    {
        ...
        public async Task<Result<string>> Handle(DeleteSheetCommand request, CancellationToken cancellationToken)
        {
            try
            {
                var sheet = await _unitOfWork.Repository<Domain.Entities.Sheet>().GetByIdAsync(request.SheetId, cancellationToken);
                if (sheet is null)
                {
                ...
                if(isDeleted)
                {
                    await _unitOfWork.Repository<Domain.Entities.Sheet>().RemoveAsync(request.SheetId, cancellationToken);
                    await _unitOfWork.Repository<Domain.Entities.Sheet>().SaveChangesAsync(cancellationToken);
